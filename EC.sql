...
CREATE TABLE `r_ec_brand` (
  ...
  `sBrand` varchar(64) NOT NULL COMMENT '品牌名称',
  `nCategoryID` int(64) DEFAULT NULL COMMENT '品牌所属的种类',
...
CREATE TABLE `r_ec_cartsku` (
  ...
  PRIMARY KEY (`nUserID`),
  KEY `PRIMARY_nUserID` (`nUserID`) USING BTREE COMMENT 's使用用户id作为主键索引'
...
CREATE TABLE `r_ec_category` (
  ...
  `sCategory` varchar(64) NOT NULL COMMENT '商品种类',
...
CREATE TABLE `r_ec_deliveryinfo` (
  ...
  `sExpressCompany` varchar(32) NOT NULL COMMENT '配送公司',
  `nDeliveryPrice` decimal(10,2) NOT NULL COMMENT '配送价格',
  `cStatus` enum('4','3','2','1') DEFAULT '1' COMMENT '配送状态:1：已收件;2：在途;3：待签收;4：已签收',
  PRIMARY KEY (`sDeliveryID`),
  KEY `PRIMARY_sDeliveryID` (`sDeliveryID`) USING BTREE COMMENT ' 使用配送编号作为主键索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- ----------------------------
-- Table structure for r_ec_imag
-- ----------------------------
DROP TABLE IF EXISTS `r_ec_imag`;
CREATE TABLE `r_ec_imag` (
  `nImageID` int(64) NOT NULL AUTO_INCREMENT,
  `nPosition` int(64) DEFAULT NULL,
  `nAttachment_content_type` enum('1','0') NOT NULL DEFAULT '0' COMMENT '附件类型：0：image；1：jpg',
  `sAttachment_file_name` varchar(32) NOT NULL COMMENT '附件名称',
  `sType` varchar(32) DEFAULT NULL,
  `sAttachment_updated_at` datetime NOT NULL,
...
CREATE TABLE `r_ec_orderinfo` (
  ...
  `cPaymentMethod` enum('3','2','1','0') NOT NULL DEFAULT '0' COMMENT '付款方式:0：信用支付;1：货到付款;2：银行卡支付;3：白条',
  ...
  `nAddressNo` int(64) NOT NULL COMMENT '收货地址。\r\n关联到R_EC_UserDeliveryAddress获取收货地址\r\n',
  ...
  `sDate_Paid` datetime NOT NULL COMMENT '订单支付时间\r\n格式:YYYY-MM-DD HH:MM:SS\r\n',
  `sDate_Completed` datetime NOT NULL COMMENT '订单完成时间',
...
CREATE TABLE `r_ec_ordersku` (
  `sOrderID` varchar(64) NOT NULL COMMENT '订单ID',
  `nProductID` int(64) NOT NULL COMMENT '商品ID',
...
-- ----------------------------
-- Table structure for r_ec_productinfo
-- ----------------------------
DROP TABLE IF EXISTS `r_ec_productinfo`;
CREATE TABLE `r_ec_productinfo` (
  `nProductID` int(64) NOT NULL AUTO_INCREMENT COMMENT '商品ID',
  `sProductName` varchar(64) NOT NULL COMMENT '商品名称',
...
CREATE TABLE `r_ec_shoppingcart` (
  ...
  `nTotalQuantity` int(64) NOT NULL COMMENT '总数量',
  `sCurrency` varchar(16) DEFAULT NULL COMMENT '货币类型',
  `nTotalPrice` decimal(10,2) DEFAULT NULL COMMENT '总价',
  PRIMARY KEY (`nUserID`),
  KEY `PRIMARY_nUserID` (`nUserID`) USING BTREE COMMENT '使用用户id作为主键索引'
...
CREATE TABLE `r_ec_sku` (
  ...
  `nInventory` int(64) DEFAULT NULL COMMENT '库存数量',
  `nDiscount` float DEFAULT NULL COMMENT '折扣',
  `sCurrency` varchar(16) DEFAULT NULL COMMENT '货币类型',
  PRIMARY KEY (`nProductID`),
  KEY `PRIMARY_nProductID` (`nProductID`) USING BTREE COMMENT '使用产品id作为主键索引'
...
CREATE TABLE `r_ec_userbankinfo` (
  ...
  `sCurrency` varchar(16) NOT NULL COMMENT '货币种类',
  PRIMARY KEY (`nUserID`),
...
CREATE TABLE `r_ec_userdeliveryaddress` (
  ...
  `sFirstName` varchar(32) NOT NULL DEFAULT '' COMMENT '收件人First Name',
  `sLastName` varchar(32) NOT NULL DEFAULT '' COMMENT '收件人Last Name',
  `sAddress` varchar(128) NOT NULL COMMENT '收货地址',
  `sCity` varchar(16) NOT NULL DEFAULT '' COMMENT '城市',
  `sProvince` varchar(16) NOT NULL DEFAULT '' COMMENT '省份',
  `sCountry` varchar(16) NOT NULL DEFAULT '' COMMENT '国家',
  `sEmail` varchar(16) NOT NULL DEFAULT '' COMMENT '收件人邮箱地址',
...
CREATE TABLE `r_ec_userinfo` (
  ...
  `sFirstName` varchar(32) NOT NULL DEFAULT '' COMMENT '用户First Name',
  `sLastName` varchar(32) NOT NULL DEFAULT '' COMMENT '用户Last Name',
  `sPhoneNo` varchar(16) NOT NULL DEFAULT '' COMMENT '手机号码\r\n格式：13800138000\r\n       (021)12345678\r\n       (0086)13800138000\r\n',
  `sEmailAddress` varchar(32) NOT NULL DEFAULT '' COMMENT '电子邮箱',
  `cGender` enum('2','1','0') NOT NULL DEFAULT '0' COMMENT '性别\r\n0：男\r\n1：女\r\n2：其它\r\n',
